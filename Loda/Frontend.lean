...
unsafe def elaborateType (stx : Syntax) : MetaM Ast.Ty := do
  match stx with
  -- Unit type “()` or “Unit”
  | `(loda_ty| Unit)  => pure (Ast.Ty.refin Ast.Ty.unit (fun _ => Ast.Expr.constBool True))
  ...
  -- Int and Bool
  | `(loda_ty| Int)        => pure (Ast.Ty.refin Ast.Ty.int (fun _ => Ast.Expr.constBool True))
  ...
  | `(loda_ty| { $T:loda_ty | $φ:term } ) => do
      ...
      let φ'   ← elaborateProp φ
      pure (Ast.Ty.refin T' (fun v => Ast.exprEq (Ast.Expr.var v) φ'))
