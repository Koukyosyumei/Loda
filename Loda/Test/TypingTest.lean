...
def mulCircuit : Ast.Circuit := {
  name   := "mul",
  inputs := ("x", Ast.Ty.refin Ast.Ty.int (Ast.Predicate.eq (Ast.Expr.constBool true))),
...
theorem mulCircuit_correct : (Ty.circuitCorrect Δ mulCircuit) := by
  ...
  set σ := envs.1
  set Γ := envs.2
  have hΓ : Γ "x" = (Ast.Ty.int.refin (Ast.Predicate.eq (Ast.Expr.constBool true))) := rfl
  have h_body := @let_binding_int_op_type_preservation "x" "x" "out" σ Δ Γ
              Ast.IntegerOp.add (Ast.Predicate.eq (Ast.Expr.constBool true))
                                (Ast.Predicate.eq (Ast.Expr.constBool true)) hΓ hΓ
  obtain ⟨vv, hv_eq⟩ := int_refintype_implies_exists_int_value σ Δ Γ "x" (Ast.Predicate.eq (Ast.Expr.constBool true)) hΓ hσ
