...
def tyenv2prop (σ : Ast.Env) (δ : Ast.CircuitEnv) (ctr : ℕ) (Γ: TyEnv) (x : String): Prop :=
  match Γ x with
...
theorem infer_type_of_v (a b: Ast.Expr) (op: Ast.IntegerOp) (σ: Ast.Env) (δ: Ast.CircuitEnv) (ctr: ℕ):
  expr2prop σ δ ctr (Ast.Expr.binRel v Ast.RelOp.eq (Ast.Expr.intExpr a op b)) → ∃ vv, Ast.eval σ δ ctr v = pure (Ast.Value.vInt vv) := by
  sorry
...
def circuit2prop (p : ℕ) (δ : Ast.CircuitEnv) (c : Ast.Circuit) : Prop :=
  ...
  let σ : Ast.Env :=
    (c.inputs.zip xs).foldl (fun σ (xy : (String × Ast.Ty) × ZMod p) =>
      let ((name, _), x) := xy; Ast.set σ name (Ast.Value.vF p x)) (fun _ => Ast.Value.vStar)
  let Γ : TyEnv :=
    (c.inputs.zip xs).foldl (fun Γ (xy : (String × Ast.Ty) × ZMod p) =>
      let ((name, τ), _) := xy; setTy Γ name τ) (fun _ => Ast.Ty.unit)
